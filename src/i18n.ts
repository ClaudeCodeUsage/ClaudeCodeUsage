...
const SETTINGS_I18N: Partial<Record<SupportedLanguage, Record<string, { label: string; help: string }>>> = {
  'de-DE': {
    ...
    'showHeatmap': { label: 'Token-Heatmap zeigen (Alle-Tab)', help: 'Standardmäßig aus. GitHub-artige Jahres-Heatmap; als SVG exportieren oder auf dein GitHub-Profil veröffentlichen.' },
    ...
    'enableSessionDelete': { label: '„Sitzung löschen“-Aktion aktivieren', help: 'Standardmäßig aus. Löschen-Button, der ein Gesprächsprotokoll in den Papierkorb verschiebt (berührt lokale Dateien).' },
  ...
  'zh-TW': {
    ...
    'showHeatmap': { label: '顯示 Token 熱力圖（全部分頁）', help: '預設關閉。全部分頁上的 GitHub 風格年度熱力圖；可匯出 SVG 或發佈到你的 GitHub 首頁。' },
    ...
    'enableSessionDelete': { label: '啟用「刪除會話」動作', help: '預設關閉。新增刪除按鈕，將對話記錄移至系統垃圾桶（會動到本機檔案）。' },
  ...
  'zh-CN': {
    ...
    'showHeatmap': { label: '显示 Token 热力图（全部标签）', help: '默认关闭。全部标签上的 GitHub 风格年度热力图；可导出 SVG 或发布到你的 GitHub 主页。' },
    ...
    'enableSessionDelete': { label: '启用“删除会话”操作', help: '默认关闭。新增删除按钮，将对话日志移至系统回收站（会动到本地文件）。' },
  ...
  'ja': {
    ...
    'showHeatmap': { label: 'トークンヒートマップを表示（すべてタブ）', help: '既定でオフ。GitHub 風の年間ヒートマップ。SVG 書き出しや GitHub プロフィールへの公開が可能。' },
    ...
    'enableSessionDelete': { label: '「セッション削除」操作を有効化', help: '既定でオフ。会話ログを OS のゴミ箱へ移す削除ボタンを追加（ローカルファイルに触れます）。' },
  ...
  'ko': {
    ...
    'enableSessionDelete': { label: '“세션 삭제” 동작 사용', help: '기본 꺼짐. 대화 로그를 OS 휴지통으로 옮기는 삭제 버튼 추가(로컬 파일에 접근).' },
  ...
  'pt-BR': {
    ...
    'showHeatmap': { label: 'Mostrar heatmap de tokens (aba Tudo)', help: 'Desligado por padrão. Heatmap anual estilo GitHub; exporte SVG ou publique no seu perfil do GitHub.' },
    ...
    'enableSessionDelete': { label: 'Ativar ação "excluir sessão"', help: 'Desligado por padrão. Botão que move o log da conversa para a lixeira do SO (mexe em arquivos locais).' },
