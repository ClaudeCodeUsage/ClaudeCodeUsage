...
const SETTINGS_I18N: Partial<Record<SupportedLanguage, Record<string, { label: string; help: string }>>> = {
  'de-DE': {
    ...
    'timezone': { label: 'Zeitzone für Daten', help: 'IANA-Zone (z.B. Asia/Hong_Kong). Legt die Tagesgrenze für die Summen (Heute/Monat/Tag) fest. Leer = System.' },
  ...
  'zh-TW': {
    ...
    'timezone': { label: '日期時區', help: 'IANA 時區（例如 Asia/Hong_Kong）。設定「今日/本月/每日」統計的日界。空白 = 系統。' },
  ...
  'zh-CN': {
    ...
    'timezone': { label: '日期时区', help: 'IANA 时区（例如 Asia/Hong_Kong）。设定「今日/本月/每日」统计的日界。空 = 系统。' },
  ...
  'ja': {
    ...
    'timezone': { label: '日付のタイムゾーン', help: 'IANA ゾーン (例: Asia/Hong_Kong)。「今日/今月/日次」合計の日付境界を設定します。空 = システム。' },
  ...
  'ko': {
    ...
    'timezone': { label: '날짜 시간대', help: 'IANA 표준 시간대 (예: Asia/Hong_Kong). 오늘/이번 달/일별 합계의 날짜 경계를 설정합니다. 비워두면 시스템.' },
  ...
  'pt-BR': {
    ...
    'timezone': { label: 'Fuso horário das datas', help: 'Zona IANA (ex.: America/Fortaleza). Define o limite do dia para os totais de hoje/mês/diários. Vazio = sistema.' },
