...
const SETTINGS_I18N: Partial<Record<SupportedLanguage, Record<string, { label: string; help: string }>>> = {
  'de-DE': {
    ...
    'showScopedWeekly': { label: 'Wöchentliches Limit pro Modell anzeigen', help: 'Hängt jede modellspezifische Wochengrenze deines Plans an (z. B. „fable 16%“) — nach den 5h / Wochenwerten. Der Name kommt von Anthropic und folgt dem jeweils begrenzten Modell. Der Tooltip listet sie immer, ob an oder aus.' },
  ...
  'zh-TW': {
    ...
    'showScopedWeekly': { label: '顯示每模型每週限制', help: '在 5 小時 / 每週數字後附加方案針對特定模型的每週上限(例如「fable 16%」)。名稱由 Anthropic 提供,因此會跟著實際受限的模型變動。無論開啟與否,提示框都會列出這些項目。' },
  ...
  'zh-CN': {
    ...
    'showScopedWeekly': { label: '显示每模型每周限制', help: '在 5 小时 / 每周数字后追加方案针对特定模型的每周上限(例如“fable 16%”)。名称来自 Anthropic,因此会随实际受限的模型变化。无论开启与否,悬浮提示都会列出这些项目。' },
  ...
  'ja': {
    ...
    'showScopedWeekly': { label: 'モデル別の週間制限を表示', help: 'プランがモデル別に計測する週間上限(例:「fable 16%」)を 5時間 / 週間の数値の後に追加します。名称は Anthropic 由来のため、実際に制限されているモデルに追従します。オン・オフに関わらずツールチップには常に表示されます。' },
  ...
  'ko': {
    ...
    'showScopedWeekly': { label: '모델별 주간 제한 표시', help: '요금제가 모델별로 측정하는 주간 한도(예: "fable 16%")를 5시간 / 주간 수치 뒤에 추가합니다. 이름은 Anthropic에서 제공하므로 실제로 제한되는 모델을 따릅니다. 켜져 있든 꺼져 있든 툴팁에는 항상 표시됩니다.' },
  ...
  'pt-BR': {
    ...
    'showScopedWeekly': { label: 'Mostrar limite semanal por modelo', help: 'Acrescenta qualquer teto semanal específico de modelo que seu plano medir (por ex. "fable 16%") após os valores de 5h / semanais. O nome vem da Anthropic, então acompanha o modelo que estiver limitado. A dica de contexto sempre os lista, ligado ou não.' },
  ...
  'id': {
    ...
    'showScopedWeekly': { label: 'Tampilkan batas mingguan per model', help: 'Menambahkan batas mingguan khusus model yang diukur paket Anda (mis. "fable 16%") setelah angka 5 jam / mingguan. Namanya berasal dari Anthropic, sehingga mengikuti model mana pun yang dibatasi. Tooltip selalu mencantumkannya, aktif maupun tidak.' },
